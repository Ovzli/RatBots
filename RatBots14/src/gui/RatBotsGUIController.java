...
public class RatBotsGUIController<T>
{
            ...
            DisplayMap displayMap, ResourceBundle res)
    {
        ...
        timer = new Timer(1000/INITIAL_FPS, new ActionListener()
        {
            ...
            {
                if(RatBotWorld.getRoundNum() < RatBotWorld.NUM_ROUNDS_IN_MATCH);
                step();
            }
        });
    ...
    public void step()
    {
        do{ //loop is for console mode... 
            parentFrame.getWorld().step();
        } while(consoleMode && ( RatBotWorld.getMoveNum() != RatBotWorld.NUM_MOVES_IN_ROUND 
                || RatBotWorld.getRoundNum() == RatBotWorld.NUM_ROUNDS_IN_MATCH) );

//        //Extra step to resolve totals for console Mode ???
//        if(consoleMode && RatBotWorld.getMoveNum() == RatBotWorld.NUM_MOVES_IN_ROUND)
//            parentFrame.getWorld().step();
        
        parentFrame.repaint();
//        if (++numStepsSoFar == numStepsToRun || RatBotWorld.getRoundNum() >= RatBotWorld.NUM_ROUNDS_IN_MATCH)
        if ( (consoleMode && RatBotWorld.getRoundNum() >= RatBotWorld.NUM_ROUNDS_IN_MATCH) ||
              RatBotWorld.getRoundNum() > RatBotWorld.NUM_ROUNDS_IN_MATCH  )
        {
            stop();
        }
